...
void enviar(int tipoDeMensaje, void* t_estructura,int fdDestinatario) {
	switch (tipoDeMensaje) {
		case (CONECXION_NODO): { // cuando se manda una conecxion de nodo

			t_nodo* unNodo = (t_nodo*) t_estructura;
			int tamanioStringIp = strlen(vg_ip_Nodo);
			size_t tamanioPayload = sizeof(vg_nodo->idNodo) + sizeof(vg_puerto_Nodo) + tamanioStringIp + sizeof(int);//el ultimo sizeof(int) es para agregarle el tamanio del string al payload
			void* payload = malloc(tamanioPayload);
			memcpy(payload,&(vg_nodo->idNodo),sizeof(int));
			size_t desplazamiento = sizeof(vg_nodo->idNodo);
			memcpy(payload+desplazamiento,&(vg_puerto_Nodo),sizeof(vg_puerto_Nodo));
			desplazamiento += sizeof(vg_puerto_Nodo);
			memcpy(payload+desplazamiento,&(tamanioStringIp),sizeof(tamanioStringIp));
			desplazamiento+= sizeof(tamanioStringIp);
			memcpy(payload+desplazamiento,vg_ip_Nodo,tamanioStringIp);

			PaqueteEnvio* unPaquete = serializar(tipoDeMensaje,payload,tamanioPayload);

			enviarPorSocket(fdDestinatario,&(unPaquete->tamanioMensaje),sizeof(int));
...
void* interpretarPaquete(Paquete* unPaquete, int fdReceptor) {
	...
	case (LECTURA): { //se debe previamente haber abierto el archivo bin del nodo
		int numeroDeBlouqe;
